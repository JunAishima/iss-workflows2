...
def slack(func):
    ...
    Send a message to mon-prefect-iss slack channel with the flow-run status.
    ...
    def end_of_run_workflow(stop_doc, api_key=None, dry_run=False):
        ...
        mon_prefect_iss = SlackWebhook.load("mon-prefect-iss")
        ...
        try:
            ...
            # Send a message to mon-prefect-iss if flow-run is successful.
            message = f":white_check_mark: {CATALOG_NAME} flow-run successful. (*{flow_run_name}*)\n ```run_start: {uid}\nscan_id: {scan_id}```"
            mon_prefect_iss.notify(message)
            return result
        except Exception as e:
            tb = traceback.format_exception_only(e)

            # Send a message to mon-prefect-iss, mon-prefect if flow-run failed.
